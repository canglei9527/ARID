 ...
 * 本地知识库检索插件（固化版）
 *
 * 将 __KB_ROOT__ 封装为三个模型工具：kb_search（全文检索）、
...
module.exports = {
  ...
  async apply(ctx) {
    ...
    ctx.effect(() => tools.register({
      name: 'kb_search',
      description: '全文关键词搜索本地知识库 __KB_ROOT__。按关键词在所有文档中检索（大小写不敏感，空格分隔的多个词任一命中即算），返回命中的文件和匹配行。适合回答"知识库里有没有提到 X""查一下 X 相关的内容"。',
      parameters: {
        type: 'object',
        properties: {
    ...
    ctx.effect(() => tools.register({
      name: 'kb_read',
      description: '读取知识库 __KB_ROOT__ 内的一份文档。path 是相对知识库根目录的路径，如 "示例/ollama-安装笔记.md"。长文档用 offset 和 max_chars 翻页读取。',
      parameters: {
        type: 'object',
        properties: {
          path: { type: 'string', description: '文档相对路径（相对 __KB_ROOT__，如 示例/ollama-安装笔记.md）' },
    ...
    ctx.effect(() => tools.register({
      name: 'kb_list',
      description: '列出本地知识库 __KB_ROOT__ 的目录结构。可选 path 只看某个子目录，max_depth 控制深度。适合先了解知识库里有什么资料再决定读哪篇。',
      parameters: {
        type: 'object',
        properties: {
